...
while [ $? -ne 0 ]; do
  echo "Trying to download Jenkins ClI client jar"
  cd "/var/jenkins_home/war/WEB-INF/"
  echo -n "."
  sleep 2
  wget "http://localhost:8080/jnlpJars/jenkins-cli.jar"
...
echo "Trying to import credentials"
java -jar jenkins-cli.jar -s $JENKINS_API -auth admin:admin create-credentials-by-xml  system::system::jenkins _ < /credentials.xml
...
for job in `ls -1 /jobs/*.xml`; do
	...
	curl -X GET $JENKINS_API/job/$JOB_NAME/ | grep "Not Found"
	if [ $? -eq 0 ]; then
...
cp -fr /credentials.xml /var/jenkins_home/credentials.xml
